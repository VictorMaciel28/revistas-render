...
import { Row, Col, Button, FormLabel, FormControl, Table, Spinner } from "react-bootstrap"
...
export default function ArticleAuthorsStep({ articleId }: { articleId: string }) {
  ...
  const handleSave = async () => {
    if (authors.length === 0 || !pdfFile) return
    setLoading(true)
    const formData = new window.FormData()
    formData.append("articleId", articleId)
    formData.append("pdfFile", pdfFile)
    formData.append("authors", JSON.stringify(authors))
    const res = await fetch("/api/magazine/article-authors", {
      method: "POST",
      body: formData,
    })
    setLoading(false)
    if (res.ok) {
      // Redireciona para o próximo passo do wizard
      router.push(`/article/wizard/${articleId}?step=2`)
    }
  }
  ...
  return (
    <>
      ...
      <Button variant="success" onClick={handleSave} disabled={authors.length === 0 || !pdfFile || loading}>
        {loading ? <Spinner size="sm" /> : "Salvar Informações"}
      </Button>
    </>
  )
} 